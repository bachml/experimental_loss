...
void CenterInnerProductLayer<Dtype>::LayerSetUp(const vector<Blob<Dtype>*>& bottom,
      const vector<Blob<Dtype>*>& top) {
  const int num_output = this->layer_param_.center_inner_product_param().num_output();  
  N_ = num_output;
  axis = bottom[0]->CanonicalAxisIndex(
      this->layer_param_.center_inner_product_param().axis());
  ...
  if (this->blobs_.size() > 0) {
    ...
    shared_ptr<Filler<Dtype> > center_filler(GetFiller<Dtype>(
        this->layer_param_.center_inner_product_param().center_filler()));
  ...
  bias_term_ = this->layer_param_.center_inner_product_param().bias_term();
  transpose_ = this->layer_param_.center_inner_product_param().transpose();

}

template <typename Dtype>
void CenterInnerProductLayer<Dtype>::Reshape(const vector<Blob<Dtype>*>& bottom,
      const vector<Blob<Dtype>*>& top) {
  ...
  //norm_center.ReshapeLike(*this->blobsp[0 ])
  //
  //
  //
...
template <typename Dtype>
void CenterInnerProductLayer<Dtype>::Forward_cpu(const vector<Blob<Dtype>*>& bottom,
    const vector<Blob<Dtype>*>& top) {
  ...
  caffe_cpu_gemm<Dtype>(CblasNoTrans, transpose_ ? CblasNoTrans : CblasTrans,
      M_, N_, K_, (Dtype)1.,
      bottom_data, center, (Dtype)0., inner_product_top_data);
  /*
  if (bias_term_) {
    caffe_cpu_gemm<Dtype>(CblasNoTrans, CblasNoTrans, M_, N_, 1, (Dtype)1.,
        bias_multiplier_.cpu_data(),
        this->blobs_[1]->cpu_data(), (Dtype)1., inner_product_top_data);
  }
  */
}
...
template <typename Dtype>
void CenterInnerProductLayer<Dtype>::Backward_cpu(const vector<Blob<Dtype>*>& top,
...
#ifdef CPU_ONLY
STUB_GPU(CenterInnerProductLayer);
#endif

INSTANTIATE_CLASS(CenterInnerProductLayer);
REGISTER_LAYER_CLASS(CenterInnerProduct);
